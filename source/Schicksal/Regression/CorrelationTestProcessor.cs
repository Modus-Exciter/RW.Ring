  ...
  public class CorrelationTestProcessor : RunBase
  {
    ...
    public override void Run()
    {
      ...
      for (int i = 0; i < m_factors.Length; i++)
      {
        ...
        var x_column = new DataColumnSample(m_table.Columns[m_factors[i]], this.GetFilter(m_factors[i]));
        var y_column = new DataColumnSample(m_table.Columns[m_result], this.GetFilter(m_factors[i]));
